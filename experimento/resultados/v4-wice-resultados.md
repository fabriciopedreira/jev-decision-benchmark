...
# V4 WiCE — replicação externa com cascata executada

Execução: 23/09/2026, 16:58:49–17:21:34 UTC (13:58:49–14:21:34 em São Paulo). [Protocolo prévio](../../protocolo-v4.md); [freeze](freeze-v4-wice.json); [abertura do gabarito](unseal-v4-wice.json); [dados analíticos](v4-wice-analysis.json). **Pesquisa, não autorização de publicação ou automação.**

**Adendo de 23/09, posterior à execução:** a [revisão independente por agentes](../../revisao-independente-v4.md) confirmou integridade e apontou limites de extrapolação, interação de cache e ganho menor na cauda da cascata. A [leitura operacional](v4-latencia-e-decisao.md) esclarece o corte arbitrário de 5× sem alterar o resultado pré-registrado. As [evidências visuais](../../docs/historico-e-limites.md) são capturas pós-execução, não prints ao vivo.

## Resultado em uma frase

No teste oficial WiCE, a cascata **executada** Jev → GPT-5.6 Luna passou os portões técnicos que haviam sido mantidos da V3 para esta comparação: concordância com o gabarito superior à Luna, menor falso suporte, 78,9% dos casos resolvidos por Jev e custo calculado 63,4% menor. Isso sustenta investigar um **piloto em sombra**, não alegar que Jev ou a cascata verificam afirmações com segurança no fluxo editorial. Jev direto **não** passou o portão de custo de 5× frente à Luna (4,41×), e sua estabilidade na V4 não foi medida.
...
| Jev direto paga nova dependência? | Custo Luna/Jev **4,41×**, abaixo do mínimo 5×; p95 4,51× mais rápido; zero falhas | **Não** no portão de custo; estabilidade V4 não avaliada |
...
- Uma execução por caso não mede repetibilidade da rota nem estabilidade após mudança de versão. A V3c mostrou que rota e classe final podem variar em repetições; a V4 não apaga esse alerta.
...
- A revisão independente por agentes foi concluída, mas ainda falta um teste em sombra com afirmações reais antes de usar o resultado como base de um artigo com posicionamento forte. O [RAGTruth](https://github.com/ParticleMedia/RAGTruth), pré-listado no protocolo como teste secundário de transferência, permanece **não executado** após [triagem do mapeamento](ragtruth-triagem-v4.md); não foi omitido por resultado desfavorável.

**Decisão arquitetural provisória:** manter revisão humana e fonte original como autoridade; considerar um piloto em sombra da cascata para sugerir prioridade de checagem, nunca aprovar afirmações. **Decisão editorial:** artigo e post continuam suspensos até auditoria e definição do que exatamente a evidência permite afirmar. O achado é interessante precisamente porque a V4 distingue uma hipótese operacional defensável de uma promessa de “verificação segura”.
